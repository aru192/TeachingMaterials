...
class PuyoPuyoStyleGame:
    ...
    def __init__(self, root):
        ...
        self.current_speed = NORMAL_SPEED   # 追記箇所３
    ...
    def draw_puyo(self):
        ...
        sy = self.puyo_y + (sub_dy * BLOCK_SIZE)
    ...
    def game_loop(self):
        if not self.is_chaining:
            self.puyo_y += self.current_speed   # コード書き換え
    ...
    def start_fast_drop(self, event):
        self.current_speed = FAST_SPEED

    def stop_fast_drop(self, event):
        self.current_speed = NORMAL_SPEED

    def hard_drop(self):
        while self.puyo_y < SCREEN_HEIGHT:
            self.puyo_y += 10
        self.puyo_y = BLOCK_SIZE * 1.0
    # 追記箇所４-ここまで
