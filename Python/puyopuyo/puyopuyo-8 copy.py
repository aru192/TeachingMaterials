...
class MinimumPuyoGame:
    ...
    def spawn_puyo(self):
        self.puyo_colors = self.puyo_queue.pop(0)
    ...
    def draw(self):
        ...
        self.canvas.create_text(field_right_x + 80, 180, text="NEXT2", fill="white", font=("Arial", 12))    # ★【1.追記】
        ...
        self.draw_puyo_unit(field_right_x + 80, 210, next2_colors[0], scale = 0.8)
        self.draw_puyo_unit(field_right_x + 80, 245, next2_colors[1], scale = 0.8)
        # ★【1.ここまで追記】
    ...
    # ★【2.ここから変更】
    def draw_puyo_unit(self, cx, cy, color, scale=1.0):
        if self.puyo_images.get(color) is not None:
            self.canvas.create_image(cx, cy, image=self.puyo_images[color])
        else:
            r = (BLOCK_SIZE / 2 - 2) * scale
